...
function Voltage_poisson_data(path::String = "./DATA/Voltage_annilus_poisson.csv", ϵ₀::Float64=8.8541878176e-12)
    ...
    analytic_func(x, y) = r(x,y)^cos(3*atan(y, x)) + (1 - r(x, y)^3)/(9*ϵ₀) #Analytic function for comparison
...
function Temp_data_hard(path::String="./DATA/Temp_data_hard.csv", X::Vector{Float64} = X_sq, Y::Vector{Float64} = Y_sq)
